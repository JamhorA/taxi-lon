...
const OPENROUTER_API_KEY = import.meta.env.VITE_OPENROUTER_GEMINI_PRO_1_5_API_KEY;

if (!OPENROUTER_API_KEY) {
  throw new Error('OpenRouter API-nyckeln saknas. Kontrollera att VITE_OPENROUTER_GEMINI_PRO_1_5_API_KEY är korrekt inställd i .env-filen.');
}

export async function extractTextWithGeminiPro15(file: File): Promise<string> {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(error.message || 'Fel vid anrop till Gemini Pro 1.5 API');
    }
    ...
    console.error('Gemini Pro 1.5 OCR error:', errorMessage);
    toast.error(`Gemini Pro 1.5: ${errorMessage}`);
    throw error;
  }
}
